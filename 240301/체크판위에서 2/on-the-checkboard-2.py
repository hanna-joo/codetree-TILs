...
3. solution : 완전탐색 - 116ms, 25MB
- 처음에 (1, 1)와 (R, C)가 다른지 확인
- 첫번째 점프 : 2행부터 R-2행까지 탐색, 2열부터 C-2열까지 탐색 -> (i행, j열)
- 두번째 점프 : i+1행부터 R-1행까지 탐색, j+1행부터 C-1열까지 탐색 -> (k행, l열)
- 점프 위치 찾았으면 유효한 점프인지 확인(W, B)
"""
